...
Route::get('/master', 'AdminController@admin_dashboard')->name('admin.dashboard')->middleware(['auth', 'admin']);
Route::group(['prefix' => 'master', 'middleware' => ['auth', 'admin']], function() {
    ...
    // uploaded files
    Route::any('/uploaded-files/file-info', 'SismooUploadController@file_info')->name('uploaded-files.info');
    Route::resource('/uploaded-files', 'SismooUploadController');
    Route::get('/uploaded-files/destroy/{id}', 'SismooUploadController@destroy')->name('uploaded-files.destroy');
